...
\begin{itemize}[leftmargin=*]
    \item \textbf{Scenario (8,500-File Fleet Churn):} We deployed 5 distinct workload profiles: \texttt{apt} upgrade (Bastion), \texttt{nginx} config (Web), \texttt{ycsb} trace (DB), \texttt{make} compilation (Fileserver), and \texttt{log} rotation (Varmail). This generated a combined benign churn of \textbf{8,500 file modifications}, followed by 5 unique rootkit injections across all nodes.
    \item \textbf{Result:} As shown in Figure~\ref{fig:churn_hist}, the distribution reflects the \textit{Reconstruction Error} of the $1\times1$ CAE compared to the Snapshot baseline (Section~\ref{design_1}). Even with 8,500 modifications, benign operations cluster strictly near zero error with negligible noise. In contrast, all 5 rootkits (Red Stars) achieve an anomaly score of \textbf{1.0}, forming clear outliers. This robust separation is achieved because the $L_\infty$ pooling captures the maximum deviation across RGB channels, where any hidden file in a system directory generates a signficant signal ($P_{hidden}=0.5, P_{path}=0.1$) that deviates from the learned baseline.
    \item \textbf{Detection \& Fatigue (Fig.~\ref{fig:churn_alerts}):}
    \begin{itemize}
        \item \textbf{AIDE:} Successfully detected all \textbf{5/5 Attacks} but generated \textbf{8,500 False Alerts}, proving that legacy FIM doesn't scale for fleet-wide maintenance. 
        \item \textbf{YARA (Tuned):} Generated \textbf{102 False Alerts} (Heuristic FP) while missing all specific LKM rootkits (0/5 detected).
        \item \textbf{ClamAV (Signature):} Successfully detected \textbf{5/5 Attacks} using the research database with zero false alerts.
        \item \textbf{DeepVis (Ours):} Achieved \textbf{5/5 Detection} with \textbf{Zero False Alerts}, demonstrating O(1) alert scaling against high-volume benign churn.
    \end{itemize}
\end{itemize}
