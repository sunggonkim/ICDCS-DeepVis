...
\noindent\textbf{Performance Components. }The first section of Table~\ref{tab:ablation} shows how I/O strategy impacts throughput. Synchronous full-file reads achieve only 1,455 files/s. Adding \texttt{io\_uring} doubles throughput to 2,910 files/s by eliminating syscall overhead. The decisive improvement comes from Header Sampling, which reads only 128 bytes per file, achieving 39,993 files/s---a 27$\times$ speedup.

\noindent\textbf{Accuracy Components. }The second section isolates detection accuracy. Using only entropy (R-channel) yields F1=0.25 due to high false positives on compressed files. Enabling Hash-Grid with $L_\infty$ pooling improves F1 to 0.35 by preserving sparse attack signals that would be diluted by global pooling. Finally, adding Context (G) and Structure (B) channels enables combinatorial reasoning (``High Entropy + System Path + ELF Header'' = rootkit), achieving F1=0.98.