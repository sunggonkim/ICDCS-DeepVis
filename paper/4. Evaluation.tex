...
We curated a comprehensive malware corpus from established open-source repositories~\cite{vxunderground, themalwarerepo}. The dataset includes:
\textbf{(1) Linux Rootkits}: Diamorphine~\cite{diamorphine} (LKM-based kernel rootkit), Azazel~\cite{azazel} and BEURK~\cite{beurk} (LD\_PRELOAD userland rootkits);
\textbf{(2) Attack Tools}: Compiled DDoS tools, reverse shells, and privilege escalation utilities;
\textbf{(3) Web Threats}: PHP webshells (e.g., c99.php) and JavaScript-based malware.
The corpus totals 37,571 files, of which 68 are active ELF binaries used for rigorous binary-level evaluation. This diverse set ensures evaluation across multiple attack vectors: kernel-level persistence, library injection, and script-based payloads.
