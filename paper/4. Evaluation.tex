...
\begin{figure}[t]
    ...
    \caption{\textbf{Real Multi-Workload Fleet vs. Stealthy Attack.} (a) Aggregated error distribution from 5 real workloads (Bastion/Apt, Web/Nginx, Build/GCC, DB/Data, App/Log) maintains stability. (b) AIDE generates 180 alerts due to valid file changes, while DeepVis maintains 0 false alerts (Scale-Invariant), isolating only the rootkit.}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Operational Efficiency (Scalability):} Figure~\ref{fig:churn_analysis}(b) compares alert fatigue.
    \begin{itemize}
        \item \textbf{AIDE} generated \textbf{180 false alerts} ($100\%$ of changed files flagged), scaling linearly with churn ($O(N)$).
        \item \textbf{DeepVis} maintained \textbf{Zero False Alerts} ($O(1)$ scaling), successfully ignoring validity-preserving changes (e.g., valid DB headers) while detecting the threat.
    \end{itemize}
\end{itemize}
