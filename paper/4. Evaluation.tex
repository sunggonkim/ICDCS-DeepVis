...
We evaluated \DeepVis across a large-scale malware collection, including 3 compiled rootkits (Diamorphine, Azazel, Beurk) and 37,571 diverse malware samples from the \texttt{MalwareSourceCode} repository. Our empirical results, summarized in Table~\ref{tab:unified_detection}, distinguish between \textit{Active Binaries} (LKM drivers, shared objects) and the \textit{Total Repository} (including dormant source code and metadata). \DeepVis achieved a 100\% recall rate across all active malicious binaries and obfuscated variants. However, for the total repository, the recall adjusted to 23.1\% as \DeepVis correctly ignores low-risk dormant scripts and non-executable metadata that do not violate system integrity constraints. In contrast, signature-based tools (ClamAV, YARA, \texttt{rkhunter}) failed against obfuscated threats (0\% recall) and exhibited near-zero detection on the custom rootkit binaries.
...
\toprule
\multirow{2}{*}{\textbf{System}} & \multicolumn{3}{c}{\textbf{Recall (Detection Rate)}} & \multicolumn{3}{c}{\textbf{False Positives}} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
& \textbf{Rootkit} & \textbf{Obfuscated} & \textbf{Polyglot} & \textbf{Update} & \textbf{Packed} & \textbf{Image} \\
\midrule
ClamAV (Sig) & 33\% & 0\% & 0\% & 0\% & 0\% & 0\% \\
AIDE (FIM) & 100\% & 100\% & 100\% & 100\% & 100\% & 100\% \\
Sysdig Falco & 100\% & 100\% & 100\% & 100\% & 0\% & 0\% \\
Set-AE~\cite{zaheer2017deepsets} & 40\% & 60\% & 20\% & 12\% & 5\% & 8\% \\
\midrule
\textbf{DeepVis} & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} & \textbf{0\%} & \textbf{0\%} & \textbf{0\%} \\
\bottomrule
\end{tabular}%
}
\end{table}
