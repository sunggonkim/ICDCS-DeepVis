...
On the global repository containing 37,387 files, \DeepVis triggered alerts on 6,036 files (Alert Rate 16.1\%). While significantly higher than the previous 0.6\% ELF-only baseline, this increase represents valid cross-platform detection: 5,587 of these alerts matched Windows PE binaries newly targeted by our updated B-channel logic. DeepVis effectively filtered out the remaining 83.9\% of dormant source code and text files. In contrast, signature-based YARA flagged only 2.7\% of the repository due to its limited rule set, missing the vast majority of binary threats.
...
\begin{table}[t]
\centering
\caption{\textbf{Unified Detection Performance.} Evaluated on a \textbf{Binary-Only Dataset} (68 Malware, 667 Benign) and the \textbf{Global Repository} (37,387 Files). \DeepVis achieves 97.1\% recall on active binary threats. The Repo Rate of 16.1\% reflects successful detection of Windows/Linux binaries across the corpus.}
...
\rowcolor{gray!10} 
\textbf{DeepVis (Full)} & \textbf{97.1\%} & \textbf{0.3\%} & \textbf{16.1\%} & Admin Tools (FP) \\
\bottomrule
