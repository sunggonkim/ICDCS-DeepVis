...
\noindent
\textbf{Analysis.} \DeepVis successfully detects 3 of 4 adversarial attack types with 100\% recall. The \textit{Low-Entropy Mimicry} attack achieves 22\% evasion rate because it specifically targets the entropy channel. This is a known limitation: if an attacker can encode their payload as low-entropy data (e.g., base64 with controlled character distribution), the CAE cannot distinguish it from legitimate text files. We discuss mitigations in Section~\ref{Discussion}.
