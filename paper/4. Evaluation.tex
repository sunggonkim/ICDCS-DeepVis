...
The corpus totals 37,386 files. For Linux-focused evaluation, we use 68 active ELF binaries including compiled rootkits and attack tools.
...
\noindent\textbf{Platform-Specific B-Channel Design.}
We implemented specialized structural analysis for each platform:
...
Table~\ref{tab:multiplatform} summarizes detection results across platform categories.

\begin{table}[t]
\centering
\caption{\textbf{Multi-Platform Detection Results.} DeepVis achieves high recall on Linux (97\%), Web (90\%), and Mobile (100\%) threats. Windows detection is lower (3.1\%) due to G-channel path heuristics being Linux-centric; adapting path patterns would improve Windows recall.}
...
\midrule
Linux (ELF/Scripts) & 305 & 296 & \textbf{97.0\%} & 0.729 & 0.300 & 0.000 \\
Windows (PE/Scripts) & 1,000 & 31 & 3.1\% & 0.572 & 0.012 & 0.003 \\
Web (PHP/JS) & 144 & 129 & \textbf{89.6\%} & 0.677 & 0.256 & 0.037 \\
...
Linux threats achieve 97\% recall primarily through G-channel (mean=0.300), as malware paths contain keywords like ``rootkit'' and ``backdoor''. Mobile threats show 100\% recall via R-channel (mean=0.935), reflecting packed/obfuscated DEX files. Web threats leverage both G-channel (0.256) and B-channel PHP detection (0.037) for 89.6\% recall. Windows detection (3.1\%) is limited because our G-channel path heuristics (\texttt{/tmp}, \texttt{/dev/shm}) are Linux-specific; adapting to Windows paths (\texttt{\textbackslash Temp}, \texttt{\textbackslash AppData}) would increase recall. This demonstrates that while the \textit{principles} of DeepVis generalize across platforms, channel-specific tuning is required for optimal performance.