...
\midrule
\textbf{Total} & -- & \textbf{90/90} & \textbf{100\%} & 0.743 \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\textbf{Analysis.} The high detection confidence stems from rootkits' inherent characteristics: packed binaries exhibit high entropy (7.5--8.0 bits/byte) in their headers, unusual file permissions, and non-standard sizes. These features map to distinctive color signatures in the tensor representation that the CAE cannot reconstruct from its learned ``normal'' manifold.
...
\noindent
\textbf{Experiment Design.} We trained \DeepVis exclusively on Ubuntu 22.04 and tested on CentOS 7 and Debian 11 without any retraining or threshold adjustment. For each target OS, we measured: (1) false positive rate on clean systems, and (2) recall on rootkit injections.

\noindent
\textbf{Results.} Table~\ref{tab:crossos} demonstrates strong generalization: 0\% FPR and 100\% recall on both unseen distributions. This is notable because CentOS 7 has 3$\times$ more files than the training source and uses different package management, directory conventions, and default configurations.

\begin{table}[t]
\centering
\caption{Cross-OS Transferability (Ubuntu-trained model, no fine-tuning)}
\label{tab:crossos}
\begin{tabular}{lccc}
\toprule
...
\noindent
\textbf{Analysis.} The \textit{Shift Invariance} property of hash-based coordinate assignment enables this generalization. File paths are mapped to coordinates via SHA-256, which produces pseudo-random distributions regardless of the source directory structure. The CAE learns to recognize \textit{patterns} of legitimate file characteristics (entropy, size, permissions) rather than memorizing specific path layouts.
...
\begin{enumerate}
    ...
    \item \textbf{Detection:} 100\% recall on three rootkit families with $L_\infty \approx 0.74$ vs. $\tau = 0.632$.
    \item \textbf{Cross-OS:} Zero-shot transfer to CentOS/Debian with 0\% FPR, 100\% recall.
    \item \textbf{$L_\infty$ Advantage:} Isolates attack signal during concurrent benign updates where MSE fails.
