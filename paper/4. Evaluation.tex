...
\noindent
\textbf{Results.} Table~\ref{tab:rootkit} shows perfect detection: 100\% recall across all 90 injections. All rootkits produced $L_\infty$ scores of 0.73--0.76, significantly above the threshold $\tau = 0.632$. The detection margin ($L_\infty - \tau \approx 0.1$) provides substantial buffer against threshold drift.

\begin{table}[t]
\centering
\caption{Rootkit Detection Performance (100 iterations per rootkit)}
\label{tab:rootkit}
\scriptsize
\begin{tabular}{llccc}
\toprule
...
\midrule
Diamorphine~\cite{diamorphine} & LKM & 100/100 & 100\% & 0.733 \\
Reptile~\cite{reptile} & Hybrid & 100/100 & 100\% & 0.760 \\
Beurk~\cite{beurk} & LD\_PRELOAD & 100/100 & 100\% & 0.735 \\
Jynx2 & LD\_PRELOAD & 100/100 & 100\% & 0.724 \\
Suterusu & LKM & 100/100 & 100\% & 0.789 \\
Azazel & LD\_PRELOAD & 100/100 & 100\% & 0.718 \\
Vlany & LD\_PRELOAD & 100/100 & 100\% & 0.741 \\
Adore-ng & LKM & 100/100 & 100\% & 0.781 \\
Nurupo & LKM & 100/100 & 100\% & 0.728 \\
KBeast & LKM & 100/100 & 100\% & 0.791 \\
\midrule
\textbf{Total} & \textbf{10 types} & \textbf{1000/1000} & \textbf{100\%} & 0.750 \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\textbf{Analysis.} We evaluated against 10 real-world Linux rootkits representing the two primary attack vectors: LKM-based kernel modules (6 rootkits) and LD\_PRELOAD-based library interposition (4 rootkits). All 1,000 attack samples were detected with 100\% recall. The high detection confidence stems from rootkits' inherent characteristics: packed binaries exhibit high entropy (7.5--8.0 bits/byte) in their headers, unusual file permissions, and non-standard sizes.
...
\noindent
\textbf{Experiment Design.} We trained \DeepVis exclusively on Ubuntu 22.04 and tested on 5 unseen distributions without any retraining or threshold adjustment. For each target OS, we measured: (1) false positive rate on clean systems (50 trials), and (2) recall on rootkit injections (50 trials).

\noindent
\textbf{Results.} Table~\ref{tab:crossos} demonstrates strong cross-platform generalization. We achieve 0\% FPR and 100\% recall across all tested distributions, including Alpine (musl-based), Fedora (RPM-based), and Arch Linux (rolling release).

\begin{table}[t]
\centering
\caption{Cross-OS Transferability (Ubuntu-trained, no fine-tuning, 50 trials)}
\label{tab:crossos}
\begin{tabular}{lccc}
\toprule
...
CentOS 7 & 3,028 & \textbf{0\%} & \textbf{100\%} \\
Debian 11 & 859 & \textbf{0\%} & \textbf{100\%} \\
Alpine 3.18 & $\sim$800 & \textbf{0\%} & \textbf{100\%} \\
Fedora 38 & $\sim$2,500 & \textbf{0\%} & \textbf{100\%} \\
Arch Linux & $\sim$1,200 & \textbf{0\%} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\textbf{Analysis.} The \textit{Shift Invariance} property of hash-based coordinate assignment enables this generalization. Despite significant differences---Alpine uses musl libc instead of glibc, Fedora uses DNF/RPM packaging, and Arch has a rolling release model---the CAE learns to recognize patterns of legitimate file characteristics rather than memorizing specific path layouts.
...
\begin{table}[t]
\centering
\caption{Adversarial Attack Detection (100 iterations per attack type)}
\label{tab:adversarial}
\begin{tabular}{lcc}
\toprule
\textbf{Attack Type} & \textbf{Detected} & \textbf{Avg $L_\infty$} \\
\midrule
Low-Entropy Mimicry & 78/100 & 0.612 \\
Parasitic Injection & 100/100 & 0.748 \\
Timestomping & 100/100 & 0.735 \\
Permission Camouflage & 100/100 & 0.742 \\
\midrule
\textbf{Overall} & \textbf{378/400 (94.5\%)} & 0.709 \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent
\textit{Parasitic Injection (100\% detected):} This attack appends a malicious payload to an existing system binary (e.g., \texttt{/usr/bin/ls}). Detection succeeds because the modification changes \textit{two} tensor channels simultaneously: (1) the file size increases by the payload size, shifting the green channel value; and (2) the header entropy increases as the appended payload introduces high-entropy bytes. Even if the attacker attempts to minimize entropy impact, the size change alone creates a detectable anomaly at the original file's coordinate.

\noindent
\textit{Timestomping (100\% detected):} The attacker backdates the \texttt{mtime} to make a malicious file appear old and legitimate. However, \DeepVis's tensor representation does \textit{not} include temporal features---only entropy, size, and permissions. Therefore, timestomping has no effect on the tensor values. The rootkit's high entropy (7.65) still produces a distinctive red channel spike that the CAE cannot reconstruct, resulting in reliable detection.

\noindent
\textit{Permission Camouflage (100\% detected):} The attacker matches the rootkit's permissions to those of benign system files (e.g., \texttt{0644} for libraries). While this makes the blue channel (permissions) appear normal, the attack fails because the entropy and size channels remain anomalous. A packed rootkit binary exhibits entropy $>7.5$, which is significantly higher than typical system libraries ($S \approx 5.5$--$6.5$). The CAE detects this single-channel anomaly through the $L_\infty$ norm's focus on the maximum deviation.

\noindent
\textit{Low-Entropy Mimicry (78\% detected):} This is the most sophisticated attack: the rootkit payload is encoded as low-entropy data (e.g., base64 with controlled character distribution) and disguised as a configuration file. With $S = 4.2$, the entropy channel appears normal. The file size (2KB) is also typical for config files, and permissions (\texttt{0644}) match expected values. As a result, all three tensor channels fall within the CAE's learned ``normal'' distribution, and the $L_\infty$ score (0.612) remains below the threshold ($\tau = 0.632$) in 22\% of cases. This represents a fundamental limitation of content-agnostic feature extraction: if an attacker can craft a payload that mimics benign statistical properties across all monitored features, the CAE cannot distinguish it from legitimate files.

\noindent
\textbf{Mitigation Discussion.} The Low-Entropy Mimicry vulnerability can be addressed through complementary defenses: (1) behavioral monitoring to detect the encoded payload's execution, (2) content-based analysis using static signatures or yara rules, or (3) extending the feature set to include structural entropy (entropy variance across file sections) rather than header-only entropy. We discuss these mitigations further in Section~\ref{Discussion}.
...
\begin{enumerate}
    ...
    \item \textbf{Detection:} 100\% recall on 10 rootkit families (1,000 samples) with $L_\infty \approx 0.75$ vs. $\tau = 0.632$.
    \item \textbf{Cross-OS:} Zero-shot transfer to 5 unseen distributions (including Alpine, Fedora, Arch) with 0\% FPR, 100\% recall.
    \item \textbf{Adversarial:} 94.5\% detection rate on 4 adversarial attack types (400 samples); low-entropy mimicry is a known limitation.
