...
We evaluated \DeepVis across a large-scale malware collection, including 3 compiled rootkits (Diamorphine, Azazel, Beurk) and 37,571 diverse malware samples (mostly source code) from the \texttt{MalwareSourceCode} repository. Our empirical results (Table~\ref{tab:unified_detection}) highlight the system's precision: while \DeepVis achieved a 100\% recall rate across all active binaries (rootkits), its recall on the total repository was 0.6\% (230 files), closely matching the count of actual executables vs. dormant source code. In contrast, YARA achieved 2.7\% by matching text strings in source files, generating noise. We also acknowledge \DeepVis failed to detect script-based threats (c99.php) in legitimate contexts (0\% recall), confirming its scope is limited to binary execution anomalies.
