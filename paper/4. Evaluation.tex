...
\begin{figure}[t]
    \centering
    \subfloat[Throughput]{
        \includegraphics[width=0.48\linewidth]{Figures/fig_final_throughput.pdf}
        \label{fig:perf_throughput}
    } \hfill
    \subfloat[Interference]{
        \includegraphics[width=0.48\linewidth]{Figures/fig_final_latency.pdf}
...
\noindent\textbf{Micro-benchmark: Scan Throughput.}
We first compare DeepVis against AIDE to demonstrate operational feasibility. AIDE performs full-file cryptographic hashing, which provides strong integrity guarantees but incurs $O(N \times Size)$ I/O complexity. This heavy I/O load often forces operators to restrict scanning to weekly maintenance windows. DeepVis, in contrast, targets continuous monitoring by reading only file headers. On a GCP High tier (c2-standard-4), DeepVis achieves a 7.7$\times$ speedup over standard AIDE. To isolate the benefits of our asynchronous architecture from the reduced I/O volume, we implemented a Partial-Hash AIDE baseline that reads only the first 128 bytes of each file. Even against this optimized baseline, DeepVis maintains a 5.4$\times$ throughput advantage. This gain confirms that the performance boost stems not just from reading less data, but from our parallel \texttt{io\_uring} pipeline, which effectively hides I/O latency through massive concurrent queuing.

\noindent\textbf{Comparison with Commercial Scanners.}
We further benchmarked DeepVis against fuzzy hashing (ssdeep) and signature scanners (ClamAV, YARA) on the full \texttt{/usr} directory (240,827 files). As shown in Figure~\ref{fig:perf_analysis}(a), traditional tools are bottlenecked by synchronous content reads (127--1,004 files/s). DeepVis achieves 39,993 files/s, representing a 40$\times$ to 215$\times$ speedup over the baselines.

\noindent\textbf{Macro-benchmark: Service Interference.}
While raw throughput is critical for coverage, the definitive constraint for sidecar security tools is interference with the primary workload. High throughput is counter-productive if it monopolizes the CPU and degrades the tenant application's Service Level Agreement (SLA). We measured the P99 latency of a co-located NGINX web server while performing a full system scan. As shown in Figure~\ref{fig:perf_analysis}(b), traditional tools severely impact system responsiveness. YARA and Heuristic engines caused degradation of +546\% and +324\% respectively due to CPU-intensive pattern matching. Even AIDE induced a +291\% latency spike (12.1ms) due to blocking I/O operations. In contrast, DeepVis maintained a P99 latency of 3,162$\mu$s, reflecting a negligible +2.0\% overhead compared to the baseline (3,100$\mu$s). This confirms that the spatial hashing and asynchronous design of DeepVis allow it to operate transparently in the background.
...
%=====================================================================
\subsection{Architectural Validation: Signal Preservation vs. Dilution (RQ3)}
\label{eval_architectural}
%=====================================================================

We explicitly validate the core architectural claim of \DeepVis: that the Hash-Grid with Local Max Pooling preserves sparse attack signals otherwise lost by Set-based Global Pooling (Set-AE). We demonstrate this through comparative visualization and quantitative Signal-to-Noise Ratio (SNR) analysis.

\begin{figure}[t]
    \centering
    % [User Note] 새로 생성한 그림을 여기에 넣으세요 (DeepVis Grid vs Set-AE Vector)
...
\noindent \textit{Visualizing the Attack Signal.}
Figure~\ref{fig:signal_comparison} compares the internal representations of the two architectures under an identical attack scenario (10 malware files injected among 10,000 benign files). The top panel illustrates the reconstruction error map of the 1$\times$1 CAE in \DeepVis. The injected malware manifests as sharp, localized peaks against the diffuse background noise. This confirms that the Hash-Grid architecture maintains the spatial isolation of anomalies. In contrast, the bottom panel depicts the global feature vector of Set-AE. The global pooling mechanism aggregates the sparse attack signal with thousands of benign signals, washing out the anomaly. The resulting vector shows no distinct features distinguishing it from a benign state, visually confirming the phenomenon of signal dilution.

\noindent \textit{Quantifying the SNR Advantage.}
To quantify this isolation, we measured the Signal-to-Noise Ratio (SNR) during a live system update (\texttt{apt upgrade}). As shown in Table~\ref{tab:snr_analysis}, the standard Global MSE metric fails to distinguish the attack. The massive number of legitimate file updates creates a high noise floor ($\mu_{noise}=0.35$) which drowns out the sparse attack signal, resulting in a negligible SNR of 1.09. This quantitative evidence supports the signal dilution flaw inherent in architectures based on global pooling.

Conversely, the Local Max logic of \DeepVis effectively filters out this diffuse noise. While the benign updates raise the global error average, the single maximum pixel value remains low (0.353). However, the rootkit injection triggers a sharp local spike (0.950). This yields a superior SNR of 2.71, providing a safety margin that remains robust against system churn.

\begin{table}[t]
\centering
\caption{Signal Preservation Analysis. Comparison of detection metrics during a live system update. Global MSE (Set-AE approach) fails to distinguish the attack from update noise (SNR $\approx$ 1), whereas Local Max (DeepVis) isolates the threat (SNR $>$ 2.7).}
\label{tab:snr_analysis}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l ccc l}
\toprule
Metric (Architecture) & Noise (Update) & Signal (Attack) & SNR & Result \\
\midrule
Global MSE (Set-AE) & 0.35 & 0.38 & 1.09 & Missed \\
\rowcolor{gray!10} 
Local Max ($L_\infty$) & 0.35 & 0.95 & 2.71 & Detected \\
\bottomrule
\end{tabular}%
}
\end{table}
...
%=====================================================================
\subsection{Hyperscale Robustness and Saturation (RQ6)}
\label{eval_robustness}
%=====================================================================

A potential concern with fixed-size grids is the impact of hash collisions as the number of files scales ($N \to \infty$). We stress-tested the system by injecting up to 204,000 files into the $128 \times 128$ grid (16,384 cells).

\noindent\textbf{Resilience to Collisions.}
Table~\ref{tab:hyperscale_saturation} presents the results. Even at \textbf{99.99\% saturation} (where every cell contains $\approx$12.45 files), \DeepVis maintained a \textbf{100\% Recall}. This robustness is due to the \textit{Max-Risk Pooling} strategy: even if a benign file (error 0.1) collides with a malicious file (error 0.9) in the same pixel, the pixel's value becomes $\max(0.1, 0.9) = 0.9$. Thus, the attack signal is never diluted by collisions, only obscured by a "worse" attacker, which still triggers an alert.

\begin{table}[t]
\centering
\caption{\textbf{Hyperscale Saturation Test.} Even when the grid is completely saturated (204K files $\to$ 16K cells), Max-Risk pooling ensures the attack signal is never diluted, maintaining 100\% recall.}
\label{tab:hyperscale_saturation}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{r c c c}
\toprule
\textbf{Files ($N$)} & \textbf{Grid Saturation} & \textbf{Avg. Collisions} & \textbf{Recall} \\
\midrule
10,000 & 45.47\% & 0.61 & 100\% \\
50,000 & 95.21\% & 3.05 & 100\% \\
100,000 & 99.87\% & 6.10 & 100\% \\
\rowcolor{gray!10} 
\textbf{204,000} & \textbf{99.99\%} & \textbf{12.45} & \textbf{100\%} \\
\bottomrule
\end{tabular}%
}
\end{table}

\noindent\textbf{Overhead Analysis.}
Our component analysis at scale (500K files) confirms that \DeepVis is effectively I/O-bound. The \texttt{io\_uring} based file reading consumes 90.1\% of the total scan time, while the hashing, tensor mapping, and CAE inference account for negligible overhead ($<3\%$). This validates that our $O(1)$ inference architecture successfully decouples detection complexity from file system size.
%=====================================================================
