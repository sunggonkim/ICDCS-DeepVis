...
\begin{figure}[t]
    ...
    \caption{\textbf{Real Multi-Workload Fleet vs. Stealthy Attack.} (a) Aggregated error distribution from 5 real workloads (Bastion/Apt, Web/Nginx, Build/GCC, DB/Data, App/Log) maintains stability. (b) Alert Fatigue vs. Detection Capability. AIDE detects the attack but floods incidents with 180 false alerts (Bastion Upgrade + Fleet Ops). DeepVis is the only solution achieving Zero False Alerts while detecting the rootkit.}
    \label{fig:churn_analysis}
\end{figure}

\noindent\textbf{Diffuse Noise vs. Sparse Signal. }DeepVis effectively distinguishes between diverse benign workloads and malicious injections. In our real-world multi-profile experiment on the GCP instance ($N=5$ workload types):
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Result:} As shown in Figure~\ref{fig:churn_analysis}(a), the cumulative error distribution from all workloads remained statistically indistinguishable from the baseline ($\mu_{err} \approx 0.33$), visualized as the stable ``Benign State'' region. No benign file (even high-entropy DB data) exceeded the anomaly threshold ($L_\infty < \tau$).
    \item \textbf{Detection \& Fatigue (Fig.~\ref{fig:churn_analysis}(b)):}
    \begin{itemize}
        \item \textbf{AIDE (Legacy FIM):} Successfully detected the rootkit (True Positive) but generated \textbf{180 False Alerts} (Linear Scaling $O(N)$), primarily driven by the Bastion system upgrade (174 alerts).
        \item \textbf{YARA (Tuned):} Generated \textbf{54 False Alerts} due to heuristic matches on system binaries, while failing to detect the stealthy rootkit (Signature Miss).
        \item \textbf{DeepVis:} Achieved the ideal operational outcome: \textbf{Zero False Alerts} ($O(1)$ Scaling) across all workloads while correctly flagging the single stealthy threat.
    \end{itemize}
\end{itemize}
