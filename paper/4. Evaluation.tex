...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/linf_distribution.pdf}
    \caption{Distribution of $L_\infty$ scores for benign variations (blue) vs. rootkit attacks (red). The threshold $\tau$ cleanly separates the two distributions, achieving perfect recall.}
    \label{fig:linf_dist}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/rootkit_linf.pdf}
    \caption{Per-rootkit $L_\infty$ scores. All 10 rootkits exceed the threshold $\tau$, confirming detection across diverse attack vectors (LKM, LD\_PRELOAD, hybrid).}
    \label{fig:rootkit_linf}
\end{figure}
...
\noindent
\textbf{Per-OS Retraining.} To validate that deployment on a new OS is practical, we trained \DeepVis directly on each target OS. Table~\ref{tab:crossos_retrain} shows that retraining dramatically reduces FPR from 100\% to near-zero.

\begin{table}[t]
\centering
\caption{FPR Reduction with Per-OS Retraining (100 trials each)}
\label{tab:crossos_retrain}
\begin{tabular}{lccccc}
\toprule
\textbf{OS} & \textbf{Files} & \textbf{Zero-shot FPR} & \textbf{Retrained FPR} & \textbf{Recall} \\
\midrule
Ubuntu 22.04 & 913 & 0\% & \textbf{0\%} & 100\% \\
CentOS 7 & 3,028 & 100\% & \textbf{0\%} & 100\% \\
Debian 11 & 859 & 100\% & \textbf{8\%} & 100\% \\
Fedora 38 & 3,329 & 100\% & \textbf{4\%} & 100\% \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Resolution Scaling Ablation (Q5)}
\label{eval_resolution}
...
\noindent
\textbf{Experiment Design.} For each resolution (64$\times$64 to 512$\times$512), we measured: (1) hash collision rate, (2) FPR/Recall with per-OS training, and (3) tensor construction time. Table~\ref{tab:resolution_ablation} presents the averaged results across Ubuntu, CentOS, Debian, and Fedora.

\begin{table}[t]
\centering
\caption{Resolution Scaling: Trade-offs (measured on Ubuntu 913 files)}
\label{tab:resolution_ablation}
\scriptsize
\begin{tabular}{lccccccc}
\toprule
\textbf{Res.} & \textbf{Pixels} & \textbf{Collision} & \textbf{Recall} & \textbf{Tensor} & \textbf{CAE} & \textbf{Total} & \textbf{Model} \\
\midrule
64$\times$64 & 4K & 20.5\% & 100\% & 4.4ms & 0.6ms & \textbf{5.0ms} & 0.1 MB \\
128$\times$128 & 16K & 6.1\% & 100\% & 2.2ms & 1.1ms & \textbf{3.3ms} & 0.5 MB \\
256$\times$256 & 65K & 1.6\% & 100\% & 2.3ms & 2.8ms & \textbf{5.1ms} & 2.1 MB \\
512$\times$512 & 262K & \textbf{0.4\%} & 100\% & 3.5ms & 7.9ms & \textbf{11.4ms} & 8.2 MB \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\textbf{Per-OS Collision Analysis.} Table~\ref{tab:resolution_peros} shows the collision rate breakdown by file system size:

\begin{table}[t]
\centering
\caption{Collision Rate by OS at Different Resolutions}
\label{tab:resolution_peros}
\begin{tabular}{lcccc}
\toprule
\textbf{OS (files)} & \textbf{64$\times$64} & \textbf{128$\times$128} & \textbf{256$\times$256} & \textbf{512$\times$512} \\
\midrule
Ubuntu (913) & 10.7\% & 3.1\% & 0.8\% & \textbf{0.1\%} \\
Debian (859) & 10.1\% & 2.9\% & 0.9\% & \textbf{0.1\%} \\
CentOS (3,028) & 29.2\% & 8.5\% & 1.6\% & \textbf{0.6\%} \\
Fedora (3,329) & 32.0\% & 10.0\% & 3.1\% & \textbf{0.8\%} \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\textbf{Analysis: Trade-offs.}
\begin{itemize}
    \item \textbf{64$\times$64:} Minimal memory (0.1 MB), but high collision rates (20--32\%) for large file systems. Suitable only for containers with $<$500 files.
    \item \textbf{128$\times$128 (default):} Best balance for typical deployments. Collision $<$10\% for 3K files, 0.5 MB model fits in Kubernetes sidecar limits.
    \item \textbf{256$\times$256:} Recommended for large file systems ($>$3K files). Reduces collision to $<$3\% with 2 MB overhead.
    \item \textbf{512$\times$512:} Near-zero collision ($<$1\%), but 8 MB model size may exceed resource-constrained environments.
\end{itemize}

\noindent
\textbf{Recommendation.} We recommend \textbf{128$\times$128} as the default for container-based deployments, with dynamic scaling to 256$\times$256 when file count exceeds 3,000. Figure~\ref{fig:resolution} visualizes this trade-off.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/resolution_collision.pdf}
    \caption{Resolution scaling impact: higher resolution reduces hash collisions, with diminishing returns after 256$\times$256. Large file systems (CentOS, Fedora) benefit most from resolution scaling.}
