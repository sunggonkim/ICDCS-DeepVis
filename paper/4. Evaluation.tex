...
\begin{figure}[t]
    \centering
    \subfloat[Error Distribution Shift]{
        \includegraphics[width=0.48\linewidth]{Figures/fig_churn_hist.pdf}
        \label{fig:churn_hist}
    } \hfill
    \subfloat[Alert Fatigue Analysis]{
        \includegraphics[width=0.48\linewidth]{Figures/fig_churn_alerts.pdf}
        \label{fig:churn_alerts}
    }
    \caption{\textbf{System Churn vs. Stealthy Attack.} (a) Real `apt` maintenance (174 files reinstalled) shifts error distribution but creates no outliers. (b) AIDE generates 174 alerts due to inode changes, whereas DeepVis generates 0 false alerts, isolating only the rootkit.}
    \label{fig:churn_analysis}
\end{figure}

\noindent\textbf{Diffuse Noise vs. Sparse Signal. }DeepVis effectively distinguishes between benign system updates and malicious injections. In our real-world experiment executing `apt-get reinstall` on the GCP instance (Rank Sum Test, $p<0.01$):
\begin{itemize}[leftmargin=*]
    \item \textbf{Scenario:} We reinstalled core system packages (modifying 174 binaries) and injected 1 stealthy rootkit.
    \item \textbf{Result:} As shown in Figure~\ref{fig:churn_hist}, the system update caused a global shift in the reconstruction error distribution ($\mu_{err}$ increases), but no pixel exceeded the anomaly threshold ($L_\infty < \tau$).
    \item \textbf{Detection:} The injected rootkit, however, created a sharp spike in the $L_\infty$ score, triggering an immediate alert.
    \item \textbf{Operational Efficiency:} Figure~\ref{fig:churn_alerts} compares alert fatigue across all baselines.
    \begin{itemize}
        \item \textbf{AIDE} generated \textbf{174 false alerts} due to inode/metadata changes, overwhelming analysts.
        \item \textbf{YARA (Tuned)} triggered \textbf{$\approx$54 false alerts} due to heuristic matches on updated system files ($FPR \approx 31\%$).
        \item \textbf{ClamAV (Tuned)} and \textbf{Set-AE} remained silent (0 alerts) but \textbf{failed to detect the stealthy rootkit} (Signal Dilution/Signature Miss).
        \item \textbf{DeepVis} was the only system to achieve \textbf{Zero False Alerts} during the maintenance window while correctly flagging the single stealthy threat, demonstrating superior signal isolation.
    \end{itemize}
\end{itemize}
