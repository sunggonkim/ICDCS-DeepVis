...
We evaluated \DeepVis across a large-scale malware collection, including 3 compiled rootkits (Diamorphine, Azazel, Beurk), 37,571 diverse malware samples from the \texttt{MalwareSourceCode} repository. Our empirical results, summarized in Table~\ref{tab:unified_detection}, indicate a critical limitation. While \DeepVis achieved a 100\% recall rate across all active malicious binaries (rootkits, ELF implants), it failed to detect script-based threats when placed in legitimate contexts. Specifically, the \textbf{c99.php webshell} (a real-world artifact) evaded detection (0\% recall) when deployed in \texttt{/var/www/html}. Because \texttt{c99.php} has standard text entropy ($R \approx 0.58$) and resides in a permitted path ($G=0.0$), it generates no anomalous features. This confirms that header-only spatial mapping is insufficient for content-based threats lacking structural or contextual anomalies. For the total repository, the recall adjusted to 23.1\% as dormant scripts were similarly ignored.

\begin{table}[t]
\centering
\caption{\textbf{Unified Detection Performance.} Verified on GCP \texttt{deepvis-mid}. DeepVis achieves 100\% recall on binaries (Rootkits) but fails on Mimicry (Webshell), clarifying the valid security scope.}
\label{tab:unified_detection}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l cccc ccc}
\toprule
\multirow{2}{*}{\textbf{System}} & \multicolumn{4}{c}{\textbf{Recall (Detection Rate)}} & \multicolumn{3}{c}{\textbf{False Positives}} \\
\cmidrule(lr){2-5} \cmidrule(lr){6-8}
& \textbf{Rootkit} & \textbf{Obfusc.} & \textbf{Polyglot} & \textbf{Webshell} & \textbf{Update} & \textbf{Packed} & \textbf{Image} \\
\midrule
ClamAV (Sig) & 33\% & 0\% & 0\% & 0\% & 0\% & 0\% & 0\% \\
AIDE (FIM) & 100\% & 100\% & 100\% & 100\% & 100\% & 100\% & 100\% \\
Sysdig Falco & 100\% & 100\% & 100\% & 0\% & 100\% & 0\% & 0\% \\
Set-AE~\cite{zaheer2017deepsets} & 40\% & 60\% & 20\% & 0\% & 12\% & 5\% & 8\% \\
\midrule
\textbf{DeepVis} & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} & \textbf{0\%} & \textbf{0\%} & \textbf{0\%} & \textbf{0\%} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[t]
\centering
\caption{\textbf{Detailed Detection Accuracy.} Case-by-case analysis revealing specific failure modes (Miss) for header-mimicking implant strategies.}
...
Webshell (Public) & \texttt{c99.php} & \texttt{/v/www/html/c99.php} & 0.58 & 0.00 & 0.00 & Miss \\
Mimicry ELF & \texttt{libc\_fake.so} & \texttt{/lib/libc\_fake.so} & 0.61 & 0.00 & 0.00 & Miss \\
Safe Path & \texttt{Implant} & \texttt{/usr/bin/implant} & 0.60 & 0.00 & 0.00 & Miss \\
\midrule
...
Table~\ref{tab:unified_detection} details the detection accuracy across 15 attack scenarios. DeepVis achieves perfect discrimination on binaries but fails on context-mimicking scripts. For instance, \texttt{Diamorphine} evaded Entropy detection ($R=0.52$) but was flagged by the G-channel ($0.60$) due to its presence in \texttt{/var/tmp}. Conversely, the \textbf{Webshell (c99.php)} in \texttt{/var/www/html} exhibited benign features ($R=0.58, G=0.0, B=0.0$), leading to a false negative. This highlights the trade-off of our header-only approach: while it scales to 40k files/s, it sacrifices visibility into deep-content payloads that do not violate structural or path constraints.

To evaluate the architectural advantage of our \textbf{Hash-Grid Parallel CAE}, we implemented a Set-based Autoencoder (Set-AE) baseline following the Deep Sets framework~\cite{zaheer2017deepsets}. Set-AE applies an MLP to each file independently and aggregates results via global max-pooling. We observe that Set-AE fails to isolate sparse threats, achieving only 40\% recall on rootkits. This poor performance stems from global feature pooling ($\max_{i} h(x_i)$), which dilutes the signal of a single malicious file ($N=1$) against the variance of thousands of benign system files ($N \approx 240K$). In contrast, our Hash-Grid Parallel CAE projects files onto a fixed Spatial Grid ($128 \times 128$), ensuring that sparse anomalies remain locally distinct ($L_\infty$ spikes) rather than being averaged out globally.

\noindent\textbf{Unsupervised Complexity vs. Simple Rules.} 
We benchmarked our 1$\times$1 CAE against simpler linear models (Logistic Regression on RGB) and non-linear rules (calibrated thresholds). While simple rules successfully flag extreme outliers, the CAE exhibited a 2.29$\times$ higher Signal-to-Noise Ratio (SNR) in isolating sparse threats within diffuse benign noise. This is because the CAE learns the \textit{joint probability} of benign feature combinations (e.g., that high entropy is normal for \texttt{.png} files in \texttt{/usr/share} but anomalous for \texttt{.ko} files in \texttt{/tmp}), enabling it to detect "impossible states" that violate learned system regularities without requiring explicit threat signatures.
...
\noindent\textbf{Interpretation of ONNX Inference.} The exported ONNX model provides a sub-millisecond inference time per system snapshot. This low-latency verification ensures that even if an attacker attempts to rapidly move or hide a file (e.g., using a chroot sandbox or a temporary mount), the high-frequency scanning of \DeepVis captures the transient state in the tensor representation, leading to immediate detection.

\noindent\textbf{Analysis of Detection Failure Modes.} 
Table~\ref{tab:unified_detection} further isolates the failure modes of baseline systems. We include AIDE (Full-Hash FIM) as the integrity gold standard. However, comparing DeepVis (header sampling) with AIDE involves a functional mismatch: AIDE prioritizes cryptographic exactness over throughput. To address this, we also evaluate Simple Heuristics (Entropy-only) as a functionally equivalent header-based baseline.
    
    Results show that while AIDE achieves 100\% recall, it generates a 100\% False Positive rate during legitimate system updates because every file change breaks the hash. In our cloud environment, a single batch update produced over 2,000 alerts.
    DeepVis succeeds where baselines fail by using Feature Orthogonality. We also validated Sysdig Falco~\cite{falco}, which detected 100\% of active rootkits via syscall interception but failed to detect any dormant payloads (0\% recall), confirming the need for static analysis. Furthermore, DeepVis ignores legitimate updates via Local Max pooling, whereas AIDE and Falco generate excessive alerts.
...
Despite this masking, \DeepVis achieved a 100\% recall rate. The detection was driven entirely by the Structural (B) channel, which identified the mismatch between the file's relocatable ELF header (\texttt{.ko}) and its presence in a binary directory, and the Entropy (R) channel for packed components. This demonstrates that \DeepVis relies on the \textit{intersection} of features (multi-modal orthogonality) rather than a single context-dependent signal, ensuring robustness even when an attacker attempts to mask the location.
