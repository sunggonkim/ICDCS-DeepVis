...
To address the limitations of gross repository statistics, we curated a precise \textbf{Binary-Only Dataset} consisting of \textbf{68 Active Malware Binaries} (including unpacked rootkits and tools) and \textbf{667 Legitimate System Binaries} from \texttt{/usr/bin}. 
As summarized in Table~\ref{tab:unified_detection}, simple Entropy-based detection failed catastrophically, achieving only \textbf{25.0\% Recall} because many modern tools (e.g., \texttt{VirTool.DDoS}) are not packed ($R<0.6$). It also suffered a \textbf{10.2\% False Positive} rate on standard tools like \texttt{gpg} and \texttt{snap}. 
In contrast, \DeepVis leveraged its Multi-modal features (Context G + Structure B) to achieve \textbf{96.0\% Recall} on the same malware set, missing only the feature-compliant \texttt{VirTool.DDoS.TCP.a}. This stark contrast (25\% vs 96%) empirically validates the necessity of our Hash-Grid architecture over single-metric heuristics.
On the global 37k-file repository, \DeepVis maintained a surgical \textbf{0.6\% Alert Rate}, filtering out dormant source code that confused YARA (2.7\%) and ignoring valid updates that triggered AIDE (100\%).
...
\begin{table}[t]
\centering
\caption{\textbf{Unified Detection Performance.} Evaluated on a rigorous \textbf{Binary-Only Dataset} (68 Malware, 667 Benign) and the \textbf{Global Repository} (37,571 Files). DeepVis achieves 96\% recall on unpacked binaries where Entropy-only fails (25\%), while maintaining near-zero FPs.}
\label{tab:unified_detection}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l cc c l}
\toprule
\multirow{2}{*}{\textbf{System}} & \textbf{Malware Recall} & \textbf{Benign FP} & \textbf{Global Selectivity} & \multicolumn{1}{c}{\textbf{Primary Failure Mode}} \\
...
Entropy-Only & \textbf{25.0\%} & \textbf{10.2\%} & 10.2\% & \textbf{Unpacked Binaries (Miss)} \\
\midrule
\rowcolor{gray!10} 
\textbf{DeepVis} & \textbf{96.0\%} & \textbf{0.1\%} & \textbf{0.6\%} & Admin Tools (FP) \\
\bottomrule
\end{tabular}%
}
\end{table}
