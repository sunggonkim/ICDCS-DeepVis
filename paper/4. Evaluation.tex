...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        ...
        \item \textbf{YARA (Tuned):} Generated \textbf{54 False Alerts} (Heuristic FP) while missing the specific LKM rootkit due to rule mismatch.
        \item \textbf{ClamAV (Signature):} Successfully detected the known rootkit using the research database (0 False Alerts). However, this relies on prior knowledge of the threat signature.
        \item \textbf{DeepVis (Ours):} Achieved \textbf{Zero False Alerts} and correctly flagged the rootkit purely via entropy anomaly, demonstrating detection of the threat class without specific signatures.
    \end{itemize}
\end{itemize}
