...
To address the limitations of gross repository statistics, we curated a precise Binary-Only Dataset consisting of 68 Active Malware Binaries (including unpacked rootkits and tools) and 667 Legitimate System Binaries from \texttt{/usr/bin}. 
As summarized in Table~\ref{tab:unified_detection}, simple Entropy-based detection failed catastrophically, achieving only 25.0\% Recall because many modern tools (e.g., \texttt{VirTool.DDoS}) are not packed ($R<0.6$). It also suffered a 10.2\% False Positive rate on standard tools like \texttt{gpg} and \texttt{snap}. 
In contrast, \DeepVis leveraged its Multi-modal features (Context G + Structure B) to achieve 96.0\% Recall on the same malware set, missing only the feature-compliant \texttt{VirTool.DDoS.TCP.a}. This stark contrast (25\% vs 96\%) empirically validates the necessity of our Hash-Grid architecture over single-metric heuristics.
On the global 37k-file repository, \DeepVis maintained a surgical 0.6\% Alert Rate, filtering out dormant source code that confused YARA (2.7\%) and ignoring valid updates that triggered AIDE (100\%).
...
\begin{table}[t]
\centering
\caption{\textbf{Unified Detection Performance.} Evaluated on a rigorous \textbf{Binary-Only Dataset} (68 Malware, 667 Benign) and the \textbf{Global Repository} (37,571 Files).}
...
Diamorphine (Rootkit) evaded Entropy ($R=0.52$) but was caught by Context ($G=0.60$) and Structure ($B=0.50$) due to its kernel-module nature in a temporary directory. Azazel (LD\_PRELOAD) was detected via Entropy ($R=1.00$) and Context ($G=0.90$) due to packing and path anomalies.
...
\begin{table}[t]
\centering
\caption{\textbf{Detailed Detection Analysis.} Removal of path column for clarity. Multi-modal RGB features catch threats that single metrics miss.}
...
LKM Rootkit & \texttt{Diamorphine} & 0.52 & \textbf{0.60} & \textbf{0.50} & \textbf{Det.} \\
LD\_PRELOAD & \texttt{Azazel} & 0.37 & \textbf{0.60} & 0.00 & \textbf{Det.} \\
Crypto Miner & \texttt{XMRig} & 0.32 & \textbf{0.60} & 0.00 & \textbf{Det.} \\
Encrypted RK & \texttt{azazel\_enc} & \textbf{1.00} & \textbf{0.90} & \textbf{0.80} & \textbf{Det.} \\
Rev. Shell & \texttt{rev\_shell} & \textbf{1.00} & \textbf{0.70} & 0.00 & \textbf{Det.} \\
Disguised ELF & \texttt{access.log} & 0.55 & 0.00 & \textbf{1.00} & \textbf{Det.} \\
\midrule
...
Admin Tool & \texttt{uwsgi} & \textbf{0.76} & 0.00 & 0.00 & \textbf{False Pos.} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\noindent\textbf{Analysis of Low-Entropy Malware (The MSE Paradox).} 
A critical finding from our real-world deployment is that many modern attacks do \textit{not} exhibit the high entropy typically associated with packing. To quantify this, we isolated a "Binary-Only" dataset of 68 malicious ELFs and 667 benign system executables from the GCP environment. As shown in Table~\ref{tab:unified_detection}, detection based solely on Entropy ($R>0.75$) yielded a poor \textbf{25.0\% Recall} (detecting only 17/68) and a high \textbf{10.2\% False Positive rate} (flagging 68/667 benign tools). DeepVis, by fusing Context (G) and Structure (B), recovered these missed threats (detecting 65/68, or 96\%) while suppressing the false positives. This empirically validates that multi-modal fusion is essential for distinguishing unpacked malware from legitimate system tools.
%=====================================================================
