...
\noindent\textbf{Target Datasets.} We curated two datasets for evaluation: (1) a \textbf{Benign Baseline} of 47,270 legitimate system binaries from Ubuntu 20.04, including 667 curated binaries (e.g., \texttt{python3}) and 46,603 files from \texttt{/usr/bin}, to rigorously measure false positives; and (2) a \textbf{Malware Corpus} of 37,387 files sourced from open repositories~\cite{vxunderground, themalwarerepo}. This corpus primarily spans Windows (35k+ files) and Web artifacts, reflecting the diversity of cloud workloads. For our primary evaluation, we isolate 68 \textit{Active} Linux ELF binaries (e.g., Diamorphine, Azazel). While numerically smaller, these represent the qualitative core of stealthy cloud threats (rootkits) that direct file-based scanners often miss. We treat the cross-platform malware as a test for \textit{OS-agnostic structural awareness}.
...
\noindent\textbf{Header Size Sensitivity. }The choice of 64--128 byte header sampling reflects an empirical trade-off between detection fidelity and I/O efficiency. Smaller headers ($<64$B) risk missing structural signatures embedded in ELF/PE headers, degrading detection of packed or obfuscated binaries. Conversely, larger headers ($>512$B) provide diminishing detection returns while significantly increasing I/O overhead: our measurements show throughput drops from $\approx$40k files/s at 128B to $\approx$8k files/s at 4KB. The 64--128B range captures critical metadata (magic bytes, section headers, entropy signatures) while avoiding the I/O bottleneck, achieving the optimal balance for hyperscale deployment.
...
\noindent\textbf{Component Time Breakdown (Table~\ref{tab:overhead_breakdown}). }
For cold-cache scans, I/O operations dominate the execution time (70--74\%). Throughput scales linearly up to 200K files ($\approx$66k/s), leveraging the Burst IOPS of the cloud storage. At 500K files, we verify a throughput drop to 20,180 files/s; this is an expected artifact of \textit{Cloud Disk Throttling}, where the sustained scanning exhausts the Burst IOPS credits, throttling performance to the disk's baseline limits. DeepVis's CPU logic (hashing, entropy) remains negligible ($<10\%$) even under this I/O saturation, confirming the architecture is I/O-bound rather than CPU-bound.
...
\noindent\textbf{Resilience to Hash Saturation. }To validate the stability of the hash mapping as the file count ($N$) exceeds the grid capacity ($W \times H$), we stress-tested the system by simulating up to 10 million files on a $128 \times 128$ grid. Table~\ref{tab:hyperscale_saturation} shows that even at 100\% saturation with over 600 collisions per pixel, the system maintains \textbf{100\% Recall} on injected malware. Unlike traditional hash tables where collisions degrade performance to $O(N)$, our Max-Risk Pooling strategy ($\text{Grid}[h] = \max(\text{Grid}[h], s)$) ensures that tensor construction remains strictly $O(1)$. Crucially, even under extreme saturation, a single high-risk anomaly dominates the pixel value due to the \texttt{max()} operation. This design makes \DeepVis inherently resistant to \textit{Decoy Attacks}---an attempt to flood the grid with benign files to mask a threat---as low-score benign files cannot suppress the signal of a malicious file. Any information loss during saturation applies exclusively to benign details, while the attack signal is preserved as the dominant feature.
...
\noindent\textbf{Component Overhead. }Component analysis at scale (500K files) confirms that the hashing and mapping process is computationally efficient. The \texttt{io\_uring} based file reading consumes 70--74\% of the total scan time (Table~\ref{tab:ablation}), while hashing, tensor mapping, and CAE inference account for negligible overhead ($<3\%$). This validates that the Hash-Grid architecture effectively decouples detection complexity from file system size without introducing computational bottlenecks.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{Figures/fig_page_aligned_sensitivity.pdf} 
    \caption{\textbf{Effect of Scan Granularity (Page-Aligned Sampling).} While detection accuracy on target Linux binaries saturates at $\approx$96B ($F_1 \approx 0.97$, consistent with Table~\ref{tab:unified_detection}), throughput remains stable up to 4KB due to OS page prefetching. We therefore adopt a \textbf{4KB Page-Aligned Sampling} strategy, maximizing feature extraction without incurring additional I/O penalty compared to smaller headers.}
...
\noindent\textbf{Detection Fidelity. }Single-channel configurations fail to achieve robust detection. Using only the R-channel (Entropy) yields an F1-score of 0.25 due to false positives from benign compressed files. The G-channel (Context) alone is insufficient as path-based features lack discriminative power for obfuscated artifacts. The B-channel (Structure) provides marginal improvement but cannot distinguish packed binaries from legitimate executables. Combining R and G without structural awareness (RG) still fails to cross the detection threshold ($L_\infty < \tau$). Only the full RGB fusion achieves the definitive F1-score of 0.96, demonstrating that \textit{multi-modal orthogonality} is essential for distinguishing malicious artifacts from legitimate system noise. This validates the design choice to encode Entropy, Context, and Structure as independent channels rather than concatenating them into a single feature vector.
