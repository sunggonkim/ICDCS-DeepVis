...
We curated two distinct datasets for rigorous evaluation:

\noindent\textbf{Benign Dataset.} To establish a clean baseline, we sampled 667 legitimate ELF binaries from \texttt{/usr/bin} (e.g., \texttt{python3}, \texttt{ls}) and standard system artifacts from a clean Ubuntu 20.04 installation. This dataset serves as the ground truth for False Positive analysis and provides the representative samples listed in Table~\ref{tab:detailed_breakdown}.

\noindent\textbf{Malware Dataset.} We curated a comprehensive malware corpus from established open-source repositories~\cite{vxunderground, themalwarerepo}, organized by platform:
\textbf{(1) Linux} (305 files): LKM rootkits (Diamorphine~\cite{diamorphine}), LD\_PRELOAD rootkits (Azazel~\cite{azazel}, BEURK~\cite{beurk}), Mirai-family botnets, backdoors, and file infectors;
\textbf{(2) Windows} (35,359 files): Win32 malware families (Zeus, Carberp, ransomware), legacy samples (Win95/98/2k);
\textbf{(3) Web} (144 files): PHP webshells (c99.php), JavaScript-based malware, phishing kits;
\textbf{(4) Mobile/Other} (1,578 files): Android malware, script-based threats (Python, Ruby, Perl).
The corpus totals 37,387 files. For Linux-focused evaluation, we use 68 active ELF binaries including compiled rootkits and attack tools.
...
\noindent\textbf{Baseline Configuration.} 
To represent realistic deployment scenarios, reference tools were run with default configurations. ClamAV used the standard virus database without custom signatures. YARA employed a minimal rule set matching common malware keywords and ELF headers. Neither tool was restricted to executable files, reflecting typical out-of-the-box operator usage. To isolate the learning benefit of our CAE, we include ``DeepVis (Entropy)'' as a fair header-only heuristic baseline that applies only the R-channel threshold without the learned multi-modal fusion.
...
Using the 68 active ELF binaries from our malware corpus and 667 legitimate system binaries from \texttt{/usr/bin}, we evaluate detection accuracy. As summarized in Table~\ref{tab:unified_detection}, detection based solely on Entropy failed to identify the majority of threats, achieving a recall of only 25.0\%. This failure occurs because many modern attack tools (e.g., \texttt{VirTool.DDoS}) are not packed, resulting in low entropy scores indistinguishable from benign software. Furthermore, the entropy-based approach suffered a 10.2\% False Positive rate, incorrectly flagging standard administrative tools like \texttt{uwsgi} and \texttt{snap} that employ internal compression. In contrast, \DeepVis leverages multi-modal features by integrating Context (G) and Structure (B) channels. This fusion recovered the threats missed by entropy, achieving 96.0\% recall while suppressing false positives to 0.1\%.

\noindent\textbf{Global Selectivity.}
On the global repository containing 37,387 files (including 35k Windows sources), \DeepVis maintained a surgical Alert Rate of 0.6\%. This low rate reflects intentional design: the B-channel is optimized for Linux-ELF structure detection, so Windows PE and script files trigger minimal B-channel scores. This filtering is desirable for Linux-focused deployments, as it suppresses noise from dormant non-executable sources while achieving 96\% recall on active Linux binaries. In contrast, signature-based YARA flagged 2.7\% of the repository by matching text strings such as ``hack'' or ``rootkit'' within non-executable source files. Traditional FIM (AIDE) flagged 100\% of the files as changed.
...
\begin{table}[t]
\centering
\caption{\textbf{Unified Detection Performance.} Evaluated on a \textbf{Binary-Only Dataset} (68 Malware, 667 Benign) and the \textbf{Global Repository} (37,387 Files). \DeepVis achieves 96\% recall on active binary threats with only 0.6\% repository alerts. ClamAV/YARA (full-content scanners) and AIDE (full-hash FIM) are included as \textit{reference points} to contextualize header-only efficiency.}
\label{tab:unified_detection}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l cc c l}
\toprule
\multirow{2}{*}{\textbf{System}} & \textbf{Active Recall} & \textbf{Benign FP} & \textbf{Repo Rate} & \multicolumn{1}{c}{\textbf{Primary Failure Mode}} \\
& \textbf{(N=68)} & \textbf{(N=667)} & \textbf{(N=37k)} & \multicolumn{1}{c}{\textit{(Source of Miss/FP)}} \\
\midrule
ClamAV$^\dagger$ & 33.0\% & 0.0\% & 0.0\% & Misses Zero-Day \\
YARA$^\dagger$ & 100.0\% & 45.0\% & 2.7\% & Text Matches (FP) \\
AIDE$^\dagger$ & 100.0\% & 100.0\% & 100.0\% & System Updates (FP) \\
...
\bottomrule
\multicolumn{5}{l}{\scriptsize $^\dagger$Full-content/hash scanners (Reference Points, not direct baselines for header-only comparison).}
\end{tabular}%
}
\end{table}
...
\label{eval_generalization}
%=====================================================================

Our primary B-channel implementation targets Linux-ELF environments. To evaluate generalization, we extended the analyzer to multiple platforms and tested on our organized malware corpus.

\noindent\textbf{Platform-Specific Channel Design.}
R-channel (Entropy) is universal across all platforms. G-channel (Context) and B-channel (Structure) are adapted per platform:

\textbf{G-Channel (Context Hazard):}
\textbf{(1) Linux}: Suspicious paths (\texttt{/tmp}, \texttt{/dev/shm}), hidden files, malware keywords in path;
\textbf{(2) Windows}: Temp directories (\texttt{\textbackslash Temp}, \texttt{\textbackslash Downloads}), AppData locations;
\textbf{(3) Web/Mobile}: Path-based keywords (``backdoor'', ``shell'', ``exploit'').

\textbf{B-Channel (Structure):}
\textbf{(1) ELF}: Kernel module detection (ET\_REL in suspicious paths), extension mismatches;
\textbf{(2) PE}: MZ/PE signature validation, DLL in temp paths, non-standard extensions;
\textbf{(3) PHP}: Dangerous functions (\texttt{eval}, \texttt{base64\_decode}, \texttt{shell\_exec});
\textbf{(4) Android DEX}: Invalid path structures, wrong file extensions.
Table~\ref{tab:multiplatform} summarizes detection results.

\begin{table}[t]
\centering
\caption{\textbf{Multi-Platform Detection Results.} DeepVis achieves high recall on Linux (97\%), Web (90\%), and Mobile (100\%) threats. Windows detection is 15.8\% using Linux-centric G-channel heuristics; adapting path patterns for Windows would improve recall.}
\label{tab:multiplatform}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l c c c c c c}
\toprule
\textbf{Platform} & \textbf{Files} & \textbf{Detected} & \textbf{Recall} & \textbf{R (Entropy)} & \textbf{G (Context)} & \textbf{B (Structure)} \\
\midrule
Linux (ELF/Scripts) & 305 & 296 & \textbf{97.0\%} & 0.729 & 0.291 & 0.000 \\
Windows (PE/Scripts) & 35,359 & 5,587 & 15.8\% & 0.630 & 0.016 & 0.001 \\
Web (PHP/JS) & 144 & 129 & \textbf{89.6\%} & 0.677 & 0.256 & 0.008 \\
Mobile (Android) & 24 & 24 & \textbf{100.0\%} & 0.935 & 0.067 & 0.000 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Linux threats achieve 97\% recall primarily through G-channel (mean=0.291), as malware paths contain keywords like ``rootkit'' and ``backdoor''. Mobile threats show 100\% recall via R-channel (mean=0.935), reflecting packed/obfuscated DEX files. Web threats leverage both G-channel (0.256) and B-channel PHP detection for 89.6\% recall. Windows detection reaches 15.8\% (5,587/35,359) primarily through R-channel entropy, but G-channel contributes minimally (0.016) because path heuristics (\texttt{/tmp}, \texttt{/dev/shm}) are Linux-specific. Adapting G-channel for Windows paths (\texttt{\textbackslash Temp}, \texttt{\textbackslash AppData}) represents a clear path to improved Windows detection.