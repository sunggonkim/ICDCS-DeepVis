...
\begin{figure}[t]
    ...
    \caption{\textbf{System Churn vs. Stealthy Attack.} (a) Massive system update (500 files modified) shifts the error distribution (Green) but does not generate outliers. Attack injection creates a distinct outlier (Red). (b) AIDE generates 500 alerts for the update, while DeepVis generates 0, isolating only the real threat.}
    \label{fig:churn_analysis}
\end{figure}

\noindent\textbf{Diffuse Noise vs. Sparse Signal. }DeepVis effectively distinguishes between benign system updates and malicious injections. In our controlled experiment simulating `apt upgrade` (Rank Sum Test, $p<0.01$):
\begin{itemize}[leftmargin=*]
    \item \textbf{Scenario:} We modified 500 binaries (simulating version updates) and injected 1 stealthy rootkit.
    ...
    \begin{itemize}
        \item \textbf{AIDE} generated \textbf{500 false alerts} during the update, overwhelming analysts.
        \item \textbf{YARA (Tuned)} triggered \textbf{155 false alerts} due to heuristic matches on system files ($FPR \approx 31\%$).
        \item \textbf{ClamAV (Tuned)} and \textbf{Set-AE} remained silent during the update (0 alerts) but \textbf{failed to detect the stealthy rootkit} (Signal Dilution/Signature Miss).
        \item \textbf{DeepVis} was the only system to achieve \textbf{Zero False Alerts} during the update window while correctly flagging the single stealthy threat, demonstrating superior signal isolation.
    \end{itemize}
\end{itemize}
