...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Result:} As shown in Figure~\ref{fig:churn_hist}, the distribution reflects the \textit{Reconstruction Error} of the $1\times1$ CAE. Benign modifications (8,500 files) cluster near zero error as they match the learned normality. In contrast, the 5 rootkits (Red Stars) exhibit varying scores from \textbf{0.2 to 0.9}. This variability arises from the \textit{Context Hazard (G)}: malware samples in deep system paths ($P_{path}=0.7$) combined with hidden status ($P_{hidden}=0.2$) reach scores $\approx 0.9$, while stealthier variations in standard config paths ($P_{path}=0.1$) score closer to $0.2$. Crucially, all samples comfortably exceed the learned threshold $\tau \approx 0.15$.
