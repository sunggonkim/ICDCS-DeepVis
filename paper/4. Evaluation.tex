...
Experiments utilize three GCP configurations: \textit{Low} (e2-micro), \textit{Mid} (e2-standard-2), and \textit{High} (c2-standard-4, NVMe SSD). To simulate production environments, the file system is populated with up to 10~million files, including system binaries (e.g., \texttt{nginx}) and random artifacts.

The experimental setup employs a \textit{Benign Baseline} of 47,270 system binaries (Ubuntu 20.04) to measure false positives, and a \textit{Malware Corpus} of 37,387 files. The malware corpus includes 68 functional Linux kernel rootkit samples derived from five major families: Diamorphine~\cite{diamorphine}, Reptile~\cite{reptile}, and variants from public malware repositories~\cite{vxunderground, ramadhanmalware}, compiled against various kernel versions to create distinct binary signatures. This corpus is supplemented by 35k+ Windows/Web artifacts as cross-platform controls. To strictly validate the structural analysis engine, feature engineering targets the Linux ELF format. Windows PE artifacts serve as a control group to test cross-platform feature orthogonality, as discussed in Section~\ref{sec:discussion}.

\noindent\textbf{Context Hazard (G) Weights.} For reproducibility, the Context channel employs a weighted path taxonomy based on the Filesystem Hierarchy Standard (FHS). System binaries in standard paths (e.g., \texttt{/usr/bin}) receive $w=0.1$, user directories receive $w=0.3$, temporary paths (\texttt{/tmp}) receive $w=0.6$, and volatile memory locations (\texttt{/dev/shm}) receive $w=0.9$. Hidden files add $+0.2$ and deep nesting ($>$5 levels) adds $+0.1$ per level. All scores are normalized to $[0,1]$ before RGB fusion. These static weights ensure deterministic scoring across deployment environments.
...
\noindent\textbf{Traditional Monitors.} Standard ClamAV is ineffective against custom rootkits (33.0\% recall) because it relies on static signatures that fail against polymorphic variants. While the tuned verification that learns signture of the malware boosts recall to 95.4\%, it remains fundamentally reactive to unseen mutations. Conversely, AIDE achieves 100\% recall but fails in practice; by flagging every file modification as a violation, it generates a 100\% alert rate during routine updates, overwhelming operators with noise. \DeepVis resolves this trade-off by learning valid system states. It attains 97.1\% recall while suppressing alerts to 0.3\%, effectively distinguishing malicious changes from legitimate system updates.
...
Deep learning models exhibit even more severe bottlenecks. MalConv requires approximately 1 second per file for inference, while Transformer-based architectures like Pack-ALM exceed 2 seconds per file. Scanning a modern Linux distribution (500k files) with Pack-ALM would take approximately 11 days, whereas DeepVis completes the same task in under 32 seconds. This throughput difference (approximately 30,000$\times$) is not merely a matter of optimization but a fundamental architectural shift: by extracting highly representative structural features from the file header, DeepVis moves the complexity from $O(N)$ model execution to $O(1)$ metadata verification, making it the only viable candidate for continuous, high-fidelity system scanning.
...
\noindent\textbf{Resilience to Saturation.} Recall remains at 100\% even as file count increases to 10~million (Table~\ref{tab:hyperscale_saturation}). Despite the grid saturation reaching 100\% with over 610 collisions per pixel, the Max-Risk Pooling strategy ensures the high-risk anomaly dominates the pixel value. Since the pooling operation is non-linear (max), even an adversary flooding the bucket with thousands of benign decoys cannot mathematically lower the pixel value contributed by a single high-risk artifact. While Max-Risk Pooling is mathematically robust against stochastic collisions, targeted pre-image attacks represent a distinct threat model. Generating a targeted collision requires knowledge of the specific grid mapping function, which depends on a secret key $K$ that is randomized per deployment. Without leakage of the mapping seed, blind adversarial flooding is statistically ineffective against the sparse $L_\infty$ signal. Grid size sensitivity analysis (128 vs. 256 vs. 512) confirmed that the default $128 \times 128$ configuration provides sufficient resolution for sparse attack detection while minimizing memory overhead.
