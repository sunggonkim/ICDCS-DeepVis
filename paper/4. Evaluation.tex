...
\begin{figure}[t!]
    ...
    \caption{\textbf{Fleet-Scale Churn \& Alert Fatigue Analysis.} (a) Comparison of False Positive alerts and Detection Recall across 8,500 modifications. (b) Distribution of \DeepVis reconstruction errors ($\mathcal{L}_\infty$). Benign churn stays strictly below the learned threshold $\tau \approx 0.43$, while malicious rootkits manifest as high-confidence outliers.}
    \label{fig:churn_analysis}
\end{figure}

\noindent\textbf{Fleet Error Stability vs. Forensic Signal. }To evaluate detection efficacy amidst heavy maintenance, we conduct a longitudinal experiment by capturing a \textbf{Snapshot (T0)} of a stable system, followed by \textbf{Real-World Churn (T1)} generated by industry-standard workload tools: \texttt{Filebench} (simulating 5,000 fileserver/varmail I/O patterns), \texttt{Nginx} (log flooding), and \texttt{SQLite} (DB transactions). We then inject 5 distinct rootkits across varied sensitive paths (\texttt{/tmp}, \texttt{/usr/bin}, \texttt{/etc}).

As shown in Figure~\ref{fig:churn_hist}, \DeepVis maintains exceptional \textbf{Fleet Error Stability}. Even after 8,500+ modifications, the reconstruction error for benign files remains clustered strictly near zero. This is because the CAE successfully generalizes the ``structural normality'' of standard logs and binaries, resulting in an error lower than the learned baseline threshold ($\tau \approx 0.43$). In contrast, malicious rootkits introduce unseen structural anomalies and hidden attributes ($P_{hidden}=0.5$) that the CAE fails to reconstruct, yielding clear outliers ($Score \approx 0.5$--$0.9$).

Figure~\ref{fig:churn_alerts} quantifies the resulting \textbf{Alert Fatigue}. While AIDE alerts on every modification (8,500 FPs), \DeepVis achieves \textbf{Zero False Positives} by filtering out recognizably normal churn. Compared to signature-based tools like ClamAV, \DeepVis achieves identical 5/5 detection recall without requiring the manual curation of research signatures (\texttt{.hdb}), demonstrating the power of baseline-aware visual reconstruction.
