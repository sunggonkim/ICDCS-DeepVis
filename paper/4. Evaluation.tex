...
\noindent
\textbf{Resolution Scaling.} The remaining FPR in Debian/Fedora results from hash collisions at 128$\times$128 resolution. Table~\ref{tab:resolution} shows that scaling to 256$\times$256 reduces collisions by 65--74\%, further reducing FPR:

\begin{table}[t]
\centering
\caption{Resolution Impact on Hash Collisions}
\label{tab:resolution}
\begin{tabular}{lcccc}
\toprule
\textbf{OS} & \textbf{Files} & \textbf{128$\times$128} & \textbf{256$\times$256} & \textbf{Reduction} \\
\midrule
Ubuntu & 913 & 2.7\% & \textbf{1.1\%} & 59\% \\
CentOS & 3,028 & 8.3\% & \textbf{2.3\%} & 72\% \\
Debian & 859 & 2.0\% & \textbf{0.7\%} & 65\% \\
Fedora & 3,329 & 9.6\% & \textbf{2.5\%} & 74\% \\
\bottomrule
\end{tabular}
\end{table}

\noindent
These results confirm that \DeepVis achieves near-zero FPR and 100\% recall across all tested distributions when trained on the target environment. For larger file systems ($>$3,000 files), we recommend 256$\times$256 resolution.
