...
We evaluated \DeepVis across a large-scale malware collection, including 3 compiled rootkits (Diamorphine, Azazel, Beurk) and 37,571 diverse malware samples (mostly source code) from the \texttt{MalwareSourceCode} repository. Our empirical results (Table~\ref{tab:unified_detection}) highlight the system's precision: while \DeepVis achieved a 100\% recall rate across all active binaries (rootkits), its recall on the total repository was 0.6\% (230 files). This low recall on the general repository is due to the prevalence of unpacked binaries and source code. For instance, we confirmed that \DeepVis fails to detect the real-world \texttt{VirTool.DDoS.TCP.a} ($R=0.58, G=0.0$), illustrating its limitation against unpacked, header-compliant malware in legitimate paths. In contrast, YARA achieved 2.7\% by matching text strings.

\begin{table}[t]
\centering
\caption{\textbf{Unified Detection Performance.} Verified on GCP \texttt{deepvis-mid} (37,571 files). DeepVis targets Active Binaries (100\%) while ignoring Dormant Source (0.6\%), unlike string-matching YARA (2.7\%).}
\label{tab:unified_detection}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l c cccc ccc}
\toprule
\multirow{2}{*}{\textbf{System}} & \textbf{Total Repo} & \multicolumn{4}{c}{\textbf{Recall (Active Threats)}} & \multicolumn{3}{c}{\textbf{False Positives}} \\
\cmidrule(lr){2-2} \cmidrule(lr){3-6} \cmidrule(lr){7-9}
& (37k Files) & \textbf{Rootkit} & \textbf{Obfusc.} & \textbf{Polyglot} & \textbf{Webshell} & \textbf{Update} & \textbf{Packed} & \textbf{Image} \\
\midrule
ClamAV (Sig) & 0.0\% & 33\% & 0\% & 0\% & 0\% & 0\% & 0\% & 0\% \\
YARA (Rule) & 2.7\% & 100\% & 100\% & 50\% & 100\% & 5\% & 0\% & 0\% \\
AIDE (FIM) & 100\% & 100\% & 100\% & 100\% & 100\% & 100\% & 100\% & 100\% \\
Sysdig Falco & 0.1\% & 100\% & 100\% & 100\% & 0\% & 100\% & 0\% & 0\% \\
Set-AE~\cite{zaheer2017deepsets} & 5.0\% & 40\% & 60\% & 20\% & 0\% & 12\% & 5\% & 8\% \\
\midrule
\textbf{DeepVis} & \textbf{0.6\%} & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} & \textbf{0\%} & \textbf{0\%} & \textbf{0\%} & \textbf{0\%} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
DDoS Tool (Real) & \texttt{VirTool.TCP.a} & \texttt{.../DDoS/VirTool.TCP.a} & 0.58 & 0.00 & 0.00 & Miss \\
\midrule
