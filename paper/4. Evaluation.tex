...
Instead of a single global detection rate, we analyze performance across the dominant platforms in our dataset. Table~\ref{tab:unified_detection} breaks down detection recall by platform. \DeepVis demonstrates exceptional performance on binary-heavy platforms, achieving 97.1\% Recall on Linux and 100.0\% on Android (Mobile), where structural anomalies (ELF/DEX headers) are distinct. On Web patterns (PHP/JS), recall is 89.6\%, driven by specific B-channel signatures. Windows detection stands at 15.8\%; while lower than Linux, this represents a significant capability boost over Linux-only baselines, successfully flagging 5,587 PE binaries that would otherwise go unnoticed.
...
\begin{table}[t]
\centering
\caption{\textbf{Unified Detection Performance.} Evaluated across four major platforms: Linux (305 files), Windows (35k), Web (144), and Mobile (24). \DeepVis achieves near-perfect recall on Linux and Mobile threats, with significant cross-platform detection capability on Web and Windows compared to Linux-only baselines.}
...
\midrule
ClamAV$^\dagger$ & 33.0\% & 0.0\% & 0.0\% & 0.0\% & 0.0\% \\
YARA$^\dagger$ & 100.0\% & 0.0\% & 18.1\% & 0.0\% & 45.0\% \\
AIDE$^\dagger$ & 100.0\% & 0.0\% & 0.0\% & 0.0\% & 100.0\% \\
Set-AE & 40.0\% & 0.0\% & 0.0\% & 0.0\% & 5.0\% \\
\midrule
DeepVis (Ent.) & 25.0\% & 14.2\% & 5.6\% & 91.7\% & 10.2\% \\
\rowcolor{gray!10} 
\textbf{DeepVis (Full)} & \textbf{97.1\%} & \textbf{15.8\%} & \textbf{89.6\%} & \textbf{100.0\%} & \textbf{0.3\%} \\
\bottomrule
\multicolumn{6}{l}{\scriptsize $^\dagger$Ref. points evaluated on Linux. (0.0\%: No detection on corresponding testbed).}
\end{tabular}%
}
\end{table}
