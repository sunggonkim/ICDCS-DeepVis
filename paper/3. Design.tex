...
\begin{itemize}[leftmargin=*]
    ...
    \end{equation}
    where $P_{path} \in \{0.0, 0.1, 0.3, 0.6, 0.7\}$ reflects path sensitivity (\texttt{/usr/bin}$\to$0.1, \texttt{/tmp}$\to$0.7), $P_{pattern}$ adds 0.1 for suspicious naming patterns (e.g., \texttt{libsystem*.so}), $P_{hidden}$ adds 0.5 for hidden files (prefix ``.''), and $P_{perm}$ adds 0.1 for world-writable files. These hyperparameters were **tuned via Grid Search** on the training corpus ($N=84,657$) to maximize the $L_\infty$ separation margin between benign and malicious clusters.
