\section{Design of \DeepVis}
\label{sec:design}



\subsection{Overall Architecture and Procedure}
\label{design_1}
...
\begin{figure}[t]
    ...
    \caption{Architecture and Procedure of \DeepVis.}
    \label{fig:overall}
\end{figure}

Figure~\ref{fig:overall} shows the overall architecture and procedure of \DeepVis. Two phases support distributed integrity verification: Snapshot and Verification.
...
\begin{figure}[t]
    ...
    \caption{Hybrid Snapshot Pipeline.}
    \label{fig:pipeline}
\end{figure}
...
\noindent\textbf{Decision Boundary Determination.} \DeepVis establishes the decision boundary as $\text{Score} > \tau$, where $\tau$ is defined as the maximum reconstruction error derived from a clean environment. Offline calibration uses benign Linux ELF binaries to derive $\tau$ as the highest $L_\infty$ score observed in the validation corpus. This threshold represents the worst-case variance. Online monitoring flags any input that exceeds this calibrated maximum as a structural anomaly and triggers alerts. Note that the evaluation focuses on Linux ELF binaries, and extending the approach to Windows PE or Android DEX requires format-specific training.
