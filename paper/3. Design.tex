...
\begin{figure*}[!htb]
    ...
    \subfloat[Normal Text File (H $\approx$ 4.2)]{
        \includegraphics[width=0.32\textwidth]{figures/Background_entrophy/Background_Normal_text.pdf}
        \label{fig:entropy_text}
    }
    \hfill
    \subfloat[System Binary / ELF (H $\approx$ 6.1)]{
        \includegraphics[width=0.32\textwidth]{figures/Background_entrophy/Background_System_binaray.pdf}
        \label{fig:entropy_elf}
    }
    \hfill
    \subfloat[Packed/Encrypted Rootkit (H $\approx$ 7.9)]{
        \includegraphics[width=0.32\textwidth]{figures/Background_entrophy/Background_Rootkit.pdf}
        \label{fig:entropy_rootkit}
    }
    \caption{Entropy-based analysis and byte patterns of different file types. We calculate Shannon entropy $H = -\sum_{i=0}^{255} p(x_i) \log_2 p(x_i)$ from the first 64 bytes of each file. (a) Text files concentrate in ASCII range (32--126) with high redundancy. (b) ELF binaries show structured headers with null padding and common opcodes. (c) Encrypted rootkits exhibit near-uniform byte distribution---the ``Attacker's Paradox'': hiding via encryption creates a statistically detectable anomaly.}
