...
To prevent benign noise from drowning out attack signals (signal dilution), Local Max Detection ($L_\infty$) isolates the highest deviation (\ding{187}). 
...
The $1 \times 1$ Convolutional Autoencoder functions as an array of shared MLPs operating simultaneously. The encoder compresses RGB channels ($3 \rightarrow 16 \rightarrow 8$) into a latent representation of benign file characteristics. Since the kernel size is $1 \times 1$, each pixel’s reconstruction depends only on its own RGB features (Entropy, Context, Structure) without spatial mixing from neighbors. Thus, the system computes the element-wise difference to generate a pixel-wise $L_2$ error map, isolating files that violate learned norms.
