...
\noindent\textbf{Handling Hash Collisions and Adversarial Attacks.} Fixed grid size ($128 \times 128$) faces inevitable hash collisions as cloud systems grow beyond 16,384 files. \DeepVis applies max-risk pooling, where each pixel retains the maximum risk value across all colliding files per RGB channel. Single malicious files dominate pixel values despite benign collisions. Adversaries may attempt filename engineering to land in low-risk coordinates. \DeepVis defeats this through a high-entropy secret key $K$ that determines coordinate mapping. Periodic key rotation reshuffles the grid without model retraining because $1 \times 1$ convolutions operate independently of spatial position. File intrinsic features (entropy, headers) remain invariant regardless of grid position, ensuring detection robustness across key rotations.
