...
\noindent
\textit{Key Rotation without Retraining.} A critical operational advantage of the $1 \times 1$ CAE design is its independence from spatial layout. Since the model learns the joint probability of per-pixel features $(R, G, B)$ rather than inter-pixel spatial patterns, changing the secret key $K$---which merely shuffles pixel locations---does not invalidate the trained decoder. Therefore, operators can periodically rotate $K$ for security hygiene without incurring any retraining cost.
