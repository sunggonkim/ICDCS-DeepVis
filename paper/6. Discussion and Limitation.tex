...
\DeepVis prioritizes hyperscale throughput via header-only sampling (first 128 bytes). While this covers 96\% of active binary threats (Section~\ref{eval_accuracy}), it inherently misses deep-payload injections in script-based attacks or polyglots. Thus, \DeepVis functions as a \textit{high-frequency first-line defense}, filtering the massive search space to identify suspicious artifacts for subsequent deep forensic analysis. Additionally, memory-resident threats (e.g., \texttt{ptrace} injections) leave no disk footprint and require complementary memory forensics.
