...
Figure~\ref{fig:motivation} compares the scalability and precision of \DeepVis against AIDE on a GCP production instance. As depicted in Figure~\ref{fig:motivation}(a), AIDE scan time increases linearly with a steep slope, exceeding 4 minutes for just 100K files. In contrast, \DeepVis leverages a parallelized asynchronous pipeline to saturate storage bandwidth, keeping scan times under 7 seconds for the same dataset. While file ingestion remains physically $O(N)$, \DeepVis achieves an effective speedup of up to 39$\times$ against traditional full-hash baselines (and 11$\times$ against optimized header scanners), and crucially, ensures that the subsequent anomaly detection (Inference) remains constant regardless of dataset size.
...
Specifically, \DeepVis (1) transforms file metadata into a fixed-size tensor using hash-based partitioning to achieve $O(1)$ inference latency, (2) utilizes a Hash-Grid Parallel CAE with Local Max detection to pinpoint sparse anomalies amidst system churn, and (3) employs an asynchronous \texttt{io\_uring} snapshot engine to maximize I/O throughput. We position \DeepVis not as a complete replacement for deep-content scanners, but as a \textbf{High-Speed First-Pass Triage Filter} that rapidly narrows the search space from millions of files to a few suspicious candidates. Our evaluation demonstrates a 10.9$\times$ throughput improvement over optimized FIM under cold cache conditions, enabling frequent whole-system verification.