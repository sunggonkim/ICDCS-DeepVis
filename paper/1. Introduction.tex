...
Cloud computing abstracts physical infrastructure into dynamic, ephemeral resources, creating an execution environment distinct from traditional on-premise systems. Ensuring workload integrity across cloud instances and large-scale HPC clusters is therefore critical, as operators must detect and prevent unauthorized modifications that may propagate across thousands of concurrently managed nodes. However, modern applications introduce a fundamental tension between security and agility, since frequent deployments, continuous updates, and elastic scaling induce massive file churn that renders traditional integrity verification models ineffective and necessitates alternative monitoring strategies.

Two primary strategies emerge: File Integrity Monitoring (FIM) and Runtime Behavioral Analysis. FIM tools including AIDE~\cite{aide} and Tripwire~\cite{tripwire} detect mutations through cryptographic hashing, while runtime monitors such as Falco~\cite{falco} and OSSEC~\cite{ossec} capture anomalies via system call tracing. Traditional approaches confront fundamental scalability constraints. FIM scan latency exhibits $O(N)$ growth with file count, incurring prohibitive I/O bottlenecks in hyperscale storage where CPU throughput exceeds storage bandwidth. Synchronous filesystem scans across millions of files generate excessive context switching and blocking I/O. Beyond performance, alert fatigue from benign updates obscures genuine threats~\cite{arp2022dos}, forcing operators to suspend monitoring during maintenance windows and creating exploitable security gaps. Our work focuses on high-speed, snapshot-based file system auditing that scales to cloud systems by avoiding full-file scans and suppressing benign update noise.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth, trim=0 0.5cm 0 0, clip]{Figures/fig_motivation_legend.pdf}
...
\caption{\DeepVis (Our Work) vs AIDE (Existing FIM).}
\label{fig:motivation}
\end{figure}



Figure~\ref{fig:motivation} contrasts the performance of AIDE and \DeepVis (our work) on a real cloud instance. As depicted, AIDE exhibits linear scan scaling, requiring four minutes to process 100K files due to synchronous I/O and full-file hash computation. In contrast, \DeepVis reads only the 4KB header per file via parallelized asynchronous I/O, achieving 121.4$\times$ acceleration over AIDE and 16.38$\times$ over a modified AIDE with an equivalent 4KB read volume. Notably, the anomaly detection latency of \DeepVis remains constant and independent of dataset size. Furthermore, \DeepVis achieves 97.1\% recall with rootkit-specific alerts, avoiding the alert fatigue inherent in AIDE’s exhaustive change reporting.
...
\begin{tabular*}{\columnwidth}{l @{\extracolsep{\fill}} m{2.5cm} c c c c}
\toprule
